...
class SimCLR(Pretraining):
  ...
  def __init__(self, hparams):
    ...
    nclasses_val = (hparams.batch_size*torch.cuda.device_count())*2-1
    print('Num of classes for NTXentLoss:', nclasses_val)
    self.criterion_val = NTXentLoss(temperature=self.hparams.temperature, gather_distributed=(torch.cuda.device_count()>1))
    if self.hparams.loss.lower() == 'remove_fn':
      ...
      self.criterion_train = NTXentLoss(self.hparams.temperature, gather_distributed=(torch.cuda.device_count()>1))
      # nclasses_train = hparams.batch_size*2-1
      nclasses_train = (hparams.batch_size*torch.cuda.device_count())*2-1
